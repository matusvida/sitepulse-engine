...
@Slf4j
public class RunScheduledWeeklyReportUseCase {
    ...
    public void run() {
        ...
        projectCatalogRepository.findAll().forEach(project -> {
            var weekStart = generateProgressReportUseCase.previousCompletedWeekStart(project, now);
            log.info("Running scheduled weekly report generation for projectId={} weekStart={}", project.getId(), weekStart);
            generateProgressReportUseCase.generateAutomaticWeekly(project.getId(), weekStart);
        });
    }
}
