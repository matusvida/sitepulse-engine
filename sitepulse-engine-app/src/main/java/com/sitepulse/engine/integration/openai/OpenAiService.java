...
public class OpenAiService {
    ...
    public List<Map<String, Object>> parsePlanMilestones(String pdfText) {
    ...
    public String generateProgressReport(List<Map<String, Object>> imageData, String metricsContext, String milestonesContext) {
        ...
        for (Map<String, Object> image : imageData) {
            contentParts.add(Map.of("type", "text", "text", "Photo date: " + image.get("date")));
            contentParts.add(Map.of(
                    "type", "image_url",
                    "image_url", Map.of("url", "data:image/jpeg;base64," + image.get("b64"), "detail", "low")
            ));
        }
        OpenAiChatResponse response = openAiFeignClient.chat(
                authorizationHeader(),
    ...
    public Map<String, Object> evaluateMilestone(String title, String expectedState, List<byte[]> images) {
        List<Map<String, Object>> contentParts = new ArrayList<>();
        contentParts.add(Map.of(
                "type", "text",
        ...
        return readMap(extractContent(response));
    }

    private String authorizationHeader() {
        if (properties.openaiApiKey() == null || properties.openaiApiKey().isBlank()) {
            throw new ApiException(HttpStatus.BAD_REQUEST, "OPENAI_API_KEY is not configured");
        }
    ...
    private String extractContent(OpenAiChatResponse response) {
        if (response == null || response.getChoices() == null || response.getChoices().isEmpty() || response.getChoices().getFirst().getMessage() == null) {
            throw new ApiException(HttpStatus.BAD_GATEWAY, "OpenAI returned an empty response");
        }
        return response.getChoices().getFirst().getMessage().getContent();
    }

    private List<Map<String, Object>> readMilestones(String json) {
        try {
            Map<String, Object> parsed = objectMapper.readValue(json, new TypeReference<>() {});
            Object milestones = parsed.getOrDefault("milestones", List.of());
            return objectMapper.convertValue(milestones, new TypeReference<>() {});
        } catch (Exception ex) {
            throw new ApiException(HttpStatus.BAD_GATEWAY, "Failed to parse milestone response");
        }
    }
    ...
    private Map<String, Object> readMap(String json) {
        try {
            ...
            throw new ApiException(HttpStatus.BAD_GATEWAY, "Failed to parse OpenAI response");
        }
    }
